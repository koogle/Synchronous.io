...
Synchronous.prototype.createObserver = function(objectToWatch, path, room) {
  var self = this;
  if(!(objectToWatch.__observerId && objectToWatch.__observerId in this.observerCallbackStore)) {
    if(typeof path === "undefined" || path === null) {
      path = [];
    }

    objectToWatch.__observerId = this.createRandomSuffix(10) + '---' + Date.now() + '---' + path;

    // Save callback
    var callback = function(changes) {
      self.observerCallback(objectToWatch, changes, path, room);
    }
    this.observerCallbackStore[objectToWatch.__observerId] = callback;
...
Synchronous.prototype.stopObserver = function(objectToWatch) {
  ...
  if(objectToWatch.__observerId in this.observerCallbackStore) {
    Object.unobserve(objectToWatch, this.observerCallbackStore[objectToWatch.__observerId]);
    delete this.observerCallbackStore[objectToWatch.__observerId];
...
Synchronous.prototype.updateValue = function(newValue, path, rootObject, room) {
  ...
  if(typeof currentObj === "object" && currentObj !== null) {
    this.createObserver(currentObj, path.concat(lastName), room);
  }
}
