...
function Synchronous(app, http) {
  ...
  this._globalStore = {};
  this._globalStoreReadonly = false;
  ...
  this._io.on('connection', function(socket) {
     ...
     self._io.to(socket.id).emit('value-changed',
      {newValue: self._globalStore, room: null, path: [], readonly: self._globalStoreReadonly});

     socket.on('value-changed', function(msg) {
       if(typeof msg.room === "undefined" || msg.room === null) {
         // If readonly don't do something
         if(self._globalStoreReadonly) {
           return;
         }
         ...
         console.log("Got update for room ", msg.room);
         if(!(msg.room in self._perClientStore) && self._namespaceStore[msg.room].readonly) {
           return;
         }
...
Synchronous.prototype._connectToRoom = function(name, clientId) {
  if(!(name in this._namespaceStore)) {
    this._namespaceStore[name] = { storeObject: {}, refCount: 0, readonly: false};
  ...
  if(!(clientId in this._namespaceClientMapping && this._namespaceClientMapping[clientId].indexOf(name) !== -1)) {
    ...
    this._io.to(clientId).emit('value-changed',
      {newValue: this._namespaceStore[name].storeObject, room: name, path: [], readonly: this._namespaceStore[name].readonly});
  }
}
...
Synchronous.prototype.allClientspaces = function() {
  return this._perClientStore;
}

Synchronous.prototype.namespace = function(name) {
  return this._namespaceStore[name].storeObject;
}

Synchronous.prototype.setNamespaceReadonly = function(name, readonly, silent) {
  this._namespaceStore[name].readonly = readonly;

  if(!silent) {
    this._io.to(name).emit('access-change', {room: name, readonly: readonly});
  }
}

Synchronous.prototype.allNamespaces = function () {
  return this._namespaceStore;
}
...
Synchronous.prototype.setGlobalspaceReadonly = function(readonly, silent) {
  this._globalStoreReadonly = readonly;

  if(!silent) {
    this._io.sockets.emit('access-change', {room: null, readonly: readonly});
  }
}
